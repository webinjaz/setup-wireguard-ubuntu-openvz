...
echo "📥 Step 3: Downloading Valid wireguard-go Binary"
mkdir -p /usr/local/bin
curl -sLo /usr/local/bin/wireguard-go https://git.zx2c4.com/wireguard-go/snapshot/wireguard-go-0.0.20220316.tar.xz
cd /usr/local/bin
tar -xf wireguard-go-0.0.20220316.tar.xz
mv wireguard-go-0.0.20220316/wireguard-go .
...
nohup /usr/local/bin/wireguard-go ${WG_INTERFACE} >/dev/null 2>&1 &
sleep 2
WG_QUICK_USERSPACE_IMPLEMENTATION="/usr/local/bin/wireguard-go" wg-quick up ${WG_INTERFACE}
...
echo "📄 Client config saved to: ${WG_CLIENT_CONF}":${WG_PORT}
AllowedIPs = 0.0.0.0/0
PersistentKeepalive = 25
EOF
