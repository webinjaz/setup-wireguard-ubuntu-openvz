...
# Supports multiple clients, UFW firewall, IP forwarding, systemd service, QR codes
...
sudo apt install -y wireguard-tools qrencode ufw curl iproute2 iptables
...
echo "PostUp = iptables -A FORWARD -i ${WG_INTERFACE} -j ACCEPT; iptables -t nat -A POSTROUTING -o venet0 -j MASQUERADE"
echo "PostDown = iptables -D FORWARD -i ${WG_INTERFACE} -j ACCEPT; iptables -t nat -D POSTROUTING -o venet0 -j MASQUERADE"
for i in $(seq 1 $CLIENT_COUNT); do
    echo "[Peer]"
